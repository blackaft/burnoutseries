...
def process_human_sources() -> int:
    ...
    for source in MAIN.humans_txt_files():
        ...
        if not destination.exists():
            raise FileNotFoundError(f"Failed to create vault markdown file: {destination}")
        source.unlink()
...
def write_index(path: Path, items: list[dict[str, str]]) -> None:
    payload = {
        ...
        "base_url": MAIN.vault_base_url("about"),
        "count": len(items),
        "items": items,
    }
...
def scan_vault_items(directory: Path) -> list[dict[str, str]]:
...
def main() -> None:
    ...
    processed_about = process_human_sources()
    about_items = scan_vault_items(MAIN.ABOUT_DIR)
