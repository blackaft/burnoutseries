...
RSS_SOURCE = ROOT.parent / "feed.rss"
RSS_URL = "https://burnoutseries.substack.com/feed.rss"
RAW_BASE_URL = "https://raw.githubusercontent.com/blackaft/burnoutseries/dev/vaults/posts/"
NAMESPACES = {
...
def fetch_rss() -> bytes:
    if RSS_SOURCE.exists() and RSS_SOURCE.read_text(encoding="utf-8").strip():
        print(f"Reading RSS from {RSS_SOURCE.relative_to(ROOT.parent)}")
        return RSS_SOURCE.read_bytes()
    print(f"Fetching RSS from {RSS_URL}")
    result = subprocess.run(["curl", "-fsSL", RSS_URL], check=True, capture_output=True)
    RSS_SOURCE.write_bytes(result.stdout)
...
def empty_posts_index() -> dict[str, Any]:
    return {"updated_at": None, "latest": None, "count": 0, "posts": []}
...
def render_post(post: dict[str, str]) -> str:
    lines = [
        ...
        f'published: {json.dumps(post["published"], ensure_ascii=False)}',
...
def merge_metadata(existing_index: dict[str, Any], feed_posts: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    existing_posts = existing_index.get("posts", [])
...
def write_posts(feed_posts: list[dict[str, str]]) -> None:
    ...
    for post in feed_posts:
        (POSTS_DIR / post_filename(post)).write_text(render_post(post), encoding="utf-8")
...
def main() -> None:
    ...
    write_json(POSTS_JSON, build_posts_json(metadata))
